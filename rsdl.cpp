...
void window::draw_rect(SDL_Rect r, RGB color)
{
    set_color(color);
    SDL_RenderDrawRect(renderer, &r);
}

void window::fill_rect(SDL_Rect r, RGB color)
{
    set_color(color);
    SDL_RenderFillRect(renderer, &r);
}

void window::draw_bmp(string filename, SDL_Rect* src,  SDL_Rect* dst)
{
	...
	if (res == NULL){
		...
		if (res==NULL)
		{
			cout<<"unable to load image with address : "<<filename<<endl;
			exit(EXIT_FAILURE);
		}
		bmp_cache.push_back(new ImageLoad(filename, res));
	}
	SDL_BlitSurface(res,   src , surface, dst);
}

void window::draw_transparent_bmp(string filename, SDL_Rect* src, SDL_Rect* dst, RGB color)
{
    SDL_Surface* res = find_in_cache(filename);
    if (res == NULL){
        res = SDL_LoadBMP(filename.c_str());
